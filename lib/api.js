...
// Access Requests — Sub-admin
export const getAccessRequests = () => apiFetch('/access-requests/');
export const getAccessRequestDetail = (id) => apiFetch(`/access-requests/${id}/`);
export const approveRequest = (id, reviewNote = '') =>
  apiFetch(`/access-requests/${id}/approve/`, { method: 'PATCH', body: { review_note: reviewNote } });
export const rejectRequest = (id, reviewNote = '') =>
  apiFetch(`/access-requests/${id}/reject/`, { method: 'PATCH', body: { review_note: reviewNote } });
...
export const getMyRequestDetail = (id) => apiFetch(`/my-requests/${id}/`);
export const cancelMyRequest = (id) =>
  apiFetch(`/my-requests/${id}/cancel/`, { method: 'POST' });

// Requester: khiếu nại request bị reject
export const disputeMyRequest = (id, disputeReason = '') =>
  apiFetch(`/my-requests/${id}/dispute/`, { method: 'POST', body: { dispute_reason: disputeReason } });

// Sub-admin: hoàn tác request (revert status)
export const revertAccessRequest = (id, revertNote = '') =>
  apiFetch(`/access-requests/${id}/revert/`, { method: 'PATCH', body: { revert_note: revertNote } });
...
export const getOwners = () => apiFetch('/auth/users/owners/');
...
// Batches — Sub-admin quản lý batch gửi owner
export const getBatches = () => apiFetch('/batches/');
export const getBatchDetail = (id) => apiFetch(`/batches/${id}/`);
export const sendBatch = (id) => apiFetch(`/batches/${id}/send/`, { method: 'POST' });

// Owner Batches — Owner xử lý item được giao
export const getOwnerBatches = () => apiFetch('/owner-batches/');
export const getOwnerBatchDetail = (id) => apiFetch(`/owner-batches/${id}/`);
export const approveItem = (batchId, itemId, ownerNote = '') =>
  apiFetch(`/owner-batches/${batchId}/approve_item/`, { method: 'PATCH', body: { item_id: itemId, owner_note: ownerNote } });
export const rejectItem = (batchId, itemId, ownerNote) =>
  apiFetch(`/owner-batches/${batchId}/reject_item/`, { method: 'PATCH', body: { item_id: itemId, owner_note: ownerNote } });
export const revertItem = (batchId, itemId) =>
  apiFetch(`/owner-batches/${batchId}/revert_item/`, { method: 'PATCH', body: { item_id: itemId } });
